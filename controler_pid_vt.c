	      ...
	      PidDirection ControllerDirection)
{
    ...
    cpid->lastTime = chVTGetSystemTimeX();
}
...
bool pidCompute(ControlerPid* cpid)
{
  ...
  const systime_t timeChange = chTimeDiffX(cpid->lastTime, now);
...
static void  pidAdjustCoeffs (ControlerPid* cpid)
{
  const float SampleTimeInSec = TIME_I2US(cpid->SampleTime) / 1e6f ;  
...
systime_t pidGetSampleTime (ControlerPid* cpid){ return cpid->SampleTime;}
