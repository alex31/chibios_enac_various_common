...
#define SBUS_FAILSAFE_BIT 3U
  ...
  typedef struct {
    /**
     * @brief   SIO driver used by SBUS.
     * @note	Only RX is required for normal SBUS operation.
     */
    SIODriver *siop;
    /** @brief RX DMA configuration. */
    SIO::DmaUserConfig rx_dma_cfg;
    /** @brief TX DMA configuration (used by sbusSend). */
    SIO::DmaUserConfig tx_dma_cfg;
    /**
  ...
  typedef struct {
    ...
    /** @brief  DMA-backed SIO wrapper instance. */
    void *sio;
    /**
