...
static const uint8_t owBufferWrite[512] = {[0 ... sizeof(owBufferWrite)-1] = 0xff};
...
bool_t oneWireReceive (OneWireDriver* drv, uint8_t *buffer, uint8_t len)
{
  ...
  st1 = chBSemWaitTimeout (&(drv->semSent), 100);
  st2 = chBSemWaitTimeout (&(drv->semReceive), 100);
  
  if ((st1 != RDY_TIMEOUT) && (st2 != RDY_TIMEOUT)) {
    for (uint32_t i=0;  i<len; i++) {
