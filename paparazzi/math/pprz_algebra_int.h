...
#include "paparazzi/math/pprz_algebra.h"
...
#define BFP_OF_REAL(_vr, _frac)    ((int32_t)((_vr)*(1<<(_frac))))
...
static inline void int32_vect2_normalize(struct Int32Vect2 *v, uint8_t frac)
{
  ...
  if (n > 0) {
    const int32_t f = BFP_OF_REAL((1.0f), frac);
    v->x = v->x * f / (int32_t)n;
    v->y = v->y * f / (int32_t)n;
  }
}
...
{
  INT32_MAT33_DIAG(*rm, TRIG_BFP_OF_REAL(1.0f), TRIG_BFP_OF_REAL(1.0f), TRIG_BFP_OF_REAL(1.0f));
}
