 ...
 */
void pprz_polyfit_double(const double* x, const double* y, int n, int p, double* c)
{
  int i,j,k;
  ...
  double _T[p + 1][1];
  double_vect_zero (S, 2*p + 1);
  MAKE_MATRIX_PTR(T, _T, p + 1);
  double_mat_zero (T, p+1, 1);
  ...
  double w[p + 1], _v[p + 1][p + 1];
  MAKE_MATRIX_PTR(v, _v, p + 1);
  pprz_svd_double(XtX, w, v, p + 1, p + 1);


  
  double _c[p + 1][1];
  MAKE_MATRIX_PTR(c_tmp, _c, p + 1);
  pprz_svd_solve_double(c_tmp, XtX, w, v, T, p + 1, p + 1, 1);
  // set output vector
  for (i = 0; i < p + 1; i++)
    c[i] = c_tmp[i][0];
