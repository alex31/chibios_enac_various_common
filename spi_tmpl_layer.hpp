...
enum class SpiOpRead{V};
enum class SpiOpWrite{V};
			    ...
			    const bool littleendian=true)
{
  ...
  const auto regaddr8b = [&] {
    if constexpr (std::is_same_v<S, SpiOpRead>)
      return regaddr | 0x80;
    else
      return regaddr;
  } ();
...
bool spiSendAny(SPIDriver &spid, 
		const RegisterAddr7b regaddr, const Tb& regVals)
{
  if constexpr (is_tuple(regVals)) {
      const auto arr = serializeTupleWithRegAdr(SpiOpWrite::V,
						regaddr,
						regVals);
      spiSend(&spid, arr.size(), arr.data());
    } else { // Not a tupple
    const auto arr = serializeIntegralWithRegAdr(SpiOpWrite::V,
						 regaddr,
						 regVals);
    spiSend(&spid, arr.size(), arr.data());
  }
...
template<typename Tb>
bool spiReceiveAny(SPIDriver &spid, 
	     const RegisterAddr7b regaddr, Tb&& regVals)
{
  ...
  spiExchange(&spid, as, arr_s.data(), arr_r.data());
...
bool spiTransInternal(SPIDriver &spid,
	      ...
	      Lasts&& ...lasts)
{
  ...
  spiSelect(&spid);
  if constexpr (std::is_same_v<S, SpiOpRead>) {
	ret &= spiReceiveAny(spid, regaddr|0x80, buff);
    } else if constexpr (std::is_same_v<S, SpiOpWrite>) {
      ret &= spiSendAny(spid, regaddr, buff);
    }
  spiUnselect(&spid);

    
  if constexpr (sizeof...(lasts) > 0) 
    ret &= spiTransInternal(spid, lasts...);
	      ...
	      Lasts&& ...lasts)
{
  ...
  spiAcquireBus(&spid);
    ...
    static_assert(std::is_same_v<S, SpiOpRead> ||
		  std::is_same_v<S, SpiOpWrite>,
		  "Type must be SpiOpRead or SpiOpWrite");

    ret = spiTransInternal(spid, sop, regaddr, buff, lasts...);
